...
def generateGraphData(forecasting_horizon, seq_len, targetTimes, targetLinks, speed):
    ...
    for n in range(len(targetTimes)):
        ...
        x.append(speed.iloc[np.arange(tidx - len_window + 1, tidx + 1)])
...
def data_gen(data_type, forecasting_horizon=12, seq_len=12):
    ...
    #print(speed.items())
    #linkIds = np.load("/home/rodr/code/mobility-baselines/DDP-GCN-main/linkIdDict.npy").item()[data_type]
    #linkIds = speed.columns
...
def data_gen_ha(data_type, forecasting_horizon=12, seq_len=12):
    ...
    ha_dates = pd.date_range(start='2018-04-01 00:00:00', end='2018-04-20 18:00:00', freq='5min', closed='left')
    #ha_dates = pd.date_range(start='2018-04-01 00:00:00', end='2018-04-24 20:25:00', freq='5min', closed='left')
    
    # compute historical averages only for train data
    xtimes = np.array([str(t) for t in speed.index])
    speed_sum = np.zeros((len(speed.columns),7,12*24))
    speed_sum_sq = np.zeros((len(speed.columns),7,12*24))
    counts = np.zeros((len(speed.columns),7,12*24))
    for dt in ha_dates:
        weekday = dt.weekday()
    ...
    # compute historical averages to remove
    mean_hist_avg = []
    std_hist_avg = []
    for dt in speed.index:
        weekday = dt.weekday()
        tod = int(dt.hour*12+dt.minute/5)
        tidx = np.where(xtimes == str(dt))[0]
        mean_hist_avg.append(historical_avg[:,weekday,tod])
        std_hist_avg.append(historical_std[:,weekday,tod])

    mean_hist_avg = np.expand_dims(np.array(mean_hist_avg), -1)
    print(mean_hist_avg.shape)
    std_hist_avg = np.expand_dims(np.array(std_hist_avg), -1)
    print(std_hist_avg.shape)
    
    print(speed.values.shape)
    
    (train_time, val_time, test_time) = datasetIdx()
    
    speed = speed - mean_hist_avg[:,:,0]
    #speed = (speed - mean_hist_avg[:,:,0]) / std_hist_avg[:,:,0]
    print("DEBUG:", mean_hist_avg[:,:,0].mean())
    ...
    # x_stats: dict, the stats for the train dataset, including the value of mean and standard deviation.
    #x_stats = {'mean': np.mean(seq_train), 'std': np.std(seq_train)}
    x_stats = {'mean': 0, 'std': 1, 'mean_ha': mean_hist_avg, 'std_ha': std_hist_avg}
