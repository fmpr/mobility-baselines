...
from utils.math_utils import evaluation_v2
...
def model_inference(sess, pred, inputs, batch_size, n_his, n_pred, step_idx, min_va_val, min_val):
    ...
    evl_val = evaluation_v2(x_val[0:len_val, step_idx + n_his, :, :], y_val, x_stats['ha_val'][0:len_val, step_idx + n_his, :, :])
    ...
    if sum(chks):
        ...
        evl_pred = evaluation_v2(x_test[0:len_pred, step_idx + n_his, :, :], y_pred, x_stats['ha_test'][0:len_pred, step_idx + n_his, :, :])
        min_val = evl_pred
...
def model_test(inputs, batch_size, n_his, n_pred, inf_mode, load_path='./output/models/'):
    ...
    with tf.Session(graph=test_graph) as test_sess:
        ...
        evl = evaluation_v2(x_test[0:len_test, step_idx + n_his, :, :], y_test, x_stats['ha_test'][0:len_test, step_idx + n_his, :, :])
